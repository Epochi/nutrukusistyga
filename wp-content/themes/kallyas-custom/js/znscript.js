...
(function ($) {
	...
	$.ZnThemeJs.prototype = {
		...
		refresh_events : function( content ) {
			...
			fw.enable_custom_recipe_recent_work_carousel( content );
			...
			// Enable Custom Recipe Slider
			fw.enable_custom_recipe_slider( content );			
			...
			// Enable custom_recipe sortable
			fw.enable_custom_recipe_sortable( content );
			...
			fw.enable_custom_recipe_recent_works2( content );
			// Enable recent works 3
			fw.enable_custom_recipe_recent_works3( content );
			...
			fw.enable_custom_recipe_readmore(content);
		...
		enable_custom_recipe_readmore : function( scope ){
			var element = scope.find('.zncr_load_more_button');
			if (element.length === 0) { return; }
			...
			element.on( 'click', function(e){
				...
				$this.addClass( 'kl-crsortable-loadmore--loading' );
				...
				$.post( ZnThemeAjax.ajaxurl, {
					...
					$this.removeClass( 'kl-crsortable-loadmore--loading' );
		...
		*/
		...
		enable_custom_recipe_recent_work_carousel : function( content ){
			var elements = content.find('.custom_recipe_recent_works1');
			if(elements && elements.length){
				$.each(elements, function(i, e){
					var self = $(e);
					if(typeof($.fn.carouFredSel) != 'undefined') {
						...
						var options = {
							...
							prev	: {
								button	: function(){return self.closest('.custom_recipe_recentwork_carousel').find('.prev');},
								key		: "left"
							},
							next	: {
								button	: function(){return self.closest('.custom_recipe_recentwork_carousel').find('.next');},
								key		: "right"
							},
							swipe: {
								onTouch: true,
		...
		enable_custom_recipe_slider : function( content ){

			var elements = content.find('.crsl-carousel__container');
			...
			if(elements && elements.length){
				$.each(elements, function(i, e){
					...
					var highlight = function(data) {
						var item = self.triggerHandler('currentVisible');
						self.children('.crsl-carousel__item').removeClass('crsl--active-item');
						item.addClass('crsl--active-item');
					};
					var unhighlight = function(data) {
						self.children('.crsl-carousel__item').removeClass('crsl--active-item');
					};
					if(typeof($.fn.carouFredSel) != 'undefined') {
						self.carouFredSel({
							...
							auto : false,
							next : {
								button: self.closest('.crsl-carousel__wrapper').find('.crsl__next'),
								key: 'right'
							},
							prev : {
								button: self.closest('.crsl-carousel__wrapper').find('.crsl__prev'),
								key: 'left'
							},
							swipe: {
								onTouch: true,
		...
		enable_custom_recipe_sortable : function( content )
		{

			var wpkznSelector = $(content).find("ul#thumbs");
			if( wpkznSelector.length === 0) { return; }

			var kl_cr_sortable = $(wpkznSelector).closest('.kl-crsortable'),
				sortbyList = kl_cr_sortable.find('#sortBy'),
				sortBy = sortbyList.is('[data-default]') ? sortbyList.attr('data-default') : 'date',
				sortDirList = kl_cr_sortable.find('#sort-direction'),
				sortAscending = sortDirList.is('[data-default]') && sortDirList.attr('data-default') == 'asc' ? true : false,
				theFilter = $(content).find('#custom_recipe-nav li.current a').attr('data-filter'),	        /* DEFAULT FILTERING CATEGORY */
				layoutMode = wpkznSelector.is('[data-layout-mode]') ? wpkznSelector.attr('data-layout-mode') : 'masonry';
			...
			//#1 Filtering
			var a_elements = $(content).find('#custom_recipe-nav li a');
			if(a_elements && a_elements.length > 0) {
				$.each(a_elements, function (a, b) {
					$(b).on('click', function (w) {
						w.preventDefault();
						$(content).find('#custom_recipe-nav li').removeClass('current');
						$(b).parent().addClass('current');
						wpkznSelector.isotope({filter: $(b).data('filter')});
						wpkznSelector.isotope('updateSortData').isotope();
					});
		...
		enable_custom_recipe_recent_works2 : function( content ){
			var elements = content.find('.custom_recipe_recent_works2');
			if(elements && elements.length){
				$.each(elements, function(i, e){
					var self = $(e);
					if(typeof($.fn.carouFredSel) != 'undefined') {
		...
		enable_custom_recipe_recent_works3 : function( content ){
			var elements = content.find('.custom_recipe_recent_works3');
			if(elements && elements.length){
				$.each(elements, function(i, e){
					var self = $(e);
					if(typeof($.fn.carouFredSel) != 'undefined') {
...
(function ($)
{
	...
	$(function($)
	{
		...
		// Check custom_recipe content
		$.each( $('.custom_recipe-item-desc-inner-compacted') , function(i, el){
			var $el = $(el),
				collapseAt = $el.is('[data-collapse-at]') && $el.attr('data-collapse-at') ? $el.attr('data-collapse-at') : 150;
			if( $el.outerHeight() < parseInt(collapseAt) ){
				$el.parent('.custom_recipe-item-desc').addClass('no-toggle');
			}
		});
		...
		if( window.matchMedia( "(min-width: 1024px)" ).matches ){
			...
			// Check custom_recipe content
			$.each( $('.custom_recipe-item-content.affixcontent') , function(i, el){

				var $el = $(el);
				var custom_recipe_page = $el.closest('.hg-custom_recipe-item');

				custom_recipe_page.imagesLoaded( function() {

					var el_size = $el.outerHeight(true),
						container_size = custom_recipe_page.outerHeight(true),
						container_offset = custom_recipe_page.offset().top,
						w_height = $(window).height(),
						max = container_size - el_size - 60,
						offset_top = 100,
						top = 0;
